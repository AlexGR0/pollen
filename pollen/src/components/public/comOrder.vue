<template>
  <div class="com">
    <!-- 全部 -->
    <div class="order" v-show="payment==1">
      ...
      <div class="box" v-for="(item, index) in data" :key="index">
        ...
        <div class="btns">
          <button
            v-show="item.paymentStatus=='-1'"
            class="pay"
            @click="pay(item.userId,item.orderId,1)"
          >支付订单</button>
    ...
    <!-- 待付款 -->
    <div class="order" v-show="payment==2">
      <div class="empty" v-show="payData==0">
        <empty></empty>
      </div>
      <div class="box" v-for="(item, index) in data" :key="index">
        <div v-show="item.paymentStatus=='-1'">
          <h5>
            订单编号：{{item.orderId}}
            <span v-if="item.paymentStatus!='-1'">交易完成</span>
            <span v-if="item.paymentStatus=='-1'">未支付</span>
    ...
    <!-- 待收货 -->
    <div class="order" v-show="payment==3">
      <div class="empty" v-show="isGetData==0">
        <empty></empty>
      </div>
      <div class="box" v-for="(item, index) in data" :key="index">
        <div v-show="item.paymentStatus=='1'">
          ...
          <div class="btns">
            <button
              v-show="item.paymentStatus=='1'"
              class="pay"
              @click="pay(item.userId,item.orderId,2)"
            >确认收货</button>
            <button class="del" @click="del(item.userId,item.orderId)">删除订单</button>
          </div>
        </div>
    ...
    <div class="order" v-show="payment==4">
      ...
      <div class="box" v-for="(item, index) in data" :key="index">
        <div v-show="item.paymentStatus=='2'||item.paymentStatus=='3'">
          <h5>
            订单编号：{{item.orderId}}
            <span v-if="item.paymentStatus!='-1'">交易完成</span>
            <span v-if="item.paymentStatus=='-1'">未支付</span>
...
export default {
  ...
  computed: {
    ...
    payData() {
      return this.data.filter(_ => _.paymentStatus == -1).length;
    },
    isGetData() {
      return this.data.filter(_ => _.paymentStatus == 1).length;
    },
    isOverData() {
      return this.data.filter(_ => _.paymentStatus == 2 || _.paymentStatus == 3)
        .length;
    }
  },
  methods: {
